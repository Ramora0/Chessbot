...
def fen_to_tensor(fen: str):
    ...
    for square, piece in board.piece_map().items():
        ...
        # Flip rank to align with standard board representation
    ...
    tensor[:, :, 14] = 1.0 if board.has_kingside_castling_rights(
        chess.WHITE) else 0.0
    tensor[:, :, 15] = 1.0 if board.has_queenside_castling_rights(
        chess.WHITE) else 0.0
    tensor[:, :, 16] = 1.0 if board.has_kingside_castling_rights(
        chess.BLACK) else 0.0
    tensor[:, :, 17] = 1.0 if board.has_queenside_castling_rights(
        chess.BLACK) else 0.0
...
class ChessBotModel(PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        dk = torch.sqrt(torch.tensor(
            self.d_model, dtype=torch.float32, device=x.device))
        ...
        if compute_loss and isinstance(inp, (list, tuple)) and len(inp) > 1:
            ...
            if true_values is not None and q_values is not None:
                ...
                loss_policy = F.cross_entropy(
                    policy.view(-1, policy.size(-1)), targets.view(-1), ignore_index=1928)

                valid_mask = (true_values.sum(dim=-1) !=
                              0) & (q_values.sum(dim=-1) != 0)
                ...
                if valid_mask.any():
                    ...
                    loss_value = F.cross_entropy(
                        valid_value_h.view(-1, valid_value_h.size(-1)), valid_z.view(-1))
                    loss_q = F.mse_loss(
                        valid_value_h_q.view(-1, valid_value_h_q.size(-1)), valid_q_values.view(-1, 3))
                else:
                    loss_value = torch.tensor(
                        0.0, device=value_h.device, requires_grad=True)
                    loss_q = torch.tensor(
                        0.0, device=value_h_q.device, requires_grad=True)
    ...
    def get_batch_position_values(self, fens, device="cuda"):
        ...
        position_tensors = []
        for fen in fens:
            x = torch.from_numpy(fen_to_tensor(fen)).to(
