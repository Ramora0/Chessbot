...
def make_model(config_dict, arch="llama"):
    if config_dict["finetuning_task"] == "text-classification":
        ...
        raise ValueError(
            f"Unknown config finetuning_task: {config_dict['finetuning_task']}")
...
def make_model_lm(config_dict, arch):
    ...
    config_dict["vocab_size"] = (
        (len(VOCAB) + len(ACTION_SPACE) + 4 + 127) // 128) * 128
    config = Config(**config_dict)
    model = Model(config=config)
    return model
        ...
        if not self._filter_illegal:
            if self.pipeline_type == "text-classification":
                ...
                for p in predictions:
                    try:
                        actions.append(
                            p[0]["generated_text"].split("[ACTION]")[-1])
                    except:
                        print(
                            "failed extracting [ACTION] from", p[0]["generated_text"])
                        actions.append("0000")
                return actions
        else:
            # TODO vectorize
            ...
            if self.pipeline_type == "text-generation":
                raise NotImplementedError(
                    "text-generation pipeline is not yet implemented for filter_illegal")
            predictions = self._pipeline(inputs)
            ...
            for i, label in enumerate(labels):
                ...
                labels[i] = [l for j, l in enumerate(
                    label) if (i, j) not in dropped_labels]
                scores[i] = [s for j, s in enumerate(
                    scores[i]) if (i, j) not in dropped_labels]
            best_moves = [max(zip(label, score), key=lambda x: x[1])[0]
